...
class Chat{
	...
	public function postNewMessage($user_name, $message, $color){
		...
		$user_name = $this->mysqli->real_escape_string($user_name);				//real_escape_string prevents SQL Injection
	...
	public function getNewMessages($id=0){
		...
		if($id>0){
			...
			FROM (SELECT message_id, user_name, message,color, DATE_FORMAT()
